 ...
 * @category Events
 */
export type VoidEventsKey = '*' | 'play' | 'pause' | 'autoplayStart'
...
/** Emitted for first time when player is mounted and first bites are ready to be player(it happens only once) */
 | 'firstReady' | 'ended' | 'mute' | 'unnmute' | 'end' | 'pipEnter' | 'pipExit' | 'fullscreenEnter' | 'fullscreenExit';
 ...
 * @category Events
 */
export declare const isShowControlsEvent: (event: unknown) => event is ShowControlsEvent;
/** Typeguard for `TimeUpdateEvent`
 * @category MediaStore
 * @category Events
 */
export declare const isTimeUpdateEvent: (event: unknown) => event is TimeUpdateEvent;
